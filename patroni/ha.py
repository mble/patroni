...
class _NodeCancellable:
    """Adapt agent cancellation to the existing async scheduler."""
    ...
    def cancel(self) -> None:
        self._node.cancel(CancelMode.NORMAL)
...
class Ha(object):
    ...
    def _node_state(self) -> NodeSnapshot:
        ...
        return self.node.snapshot(
            ...
            Freshness.FRESH,
            ObservationContext(self._leader_timeline),
        )

    def _pg_role(self, snapshot: Optional[NodeSnapshot] = None) -> Union[PostgresqlRole, str]:
        role = (snapshot or self._node_state()).desired_role
        return role.value if role == ControlPostgresRole.UNKNOWN else PostgresqlRole(role.value)

    def _pg_state(self, snapshot: Optional[NodeSnapshot] = None) -> Union[PostgresqlState, str]:
        state = (snapshot or self._node_state()).postgres_state
        return state.value if state == ControlPostgresState.UNKNOWN else PostgresqlState(state.value)

    def _pending_reason(self, snapshot: Optional[NodeSnapshot] = None) -> Dict[str, Dict[str, object]]:
        pending = (snapshot or self._node_state()).pending_restart
        return {
    ...
    def _reset_recovery(self) -> Optional[bool]:
        return self._run_command(self._lifecycle(CommandKind.RESET_RECOVERY))

    def _trigger_recovery(self) -> Optional[bool]:
        return self._run_command(self._lifecycle(CommandKind.CHECK_DIVERGENCE))

    def _callback(self, kind: CallbackKind) -> Optional[bool]:
        return self._run_command(self._lifecycle(CommandKind.CALLBACK, callback=kind))
    ...
    @staticmethod
    def _recovery_target(member: Union[Leader, Member, None]) -> Optional[RecoveryTarget]:
        checkpoint = member.checkpoint_after_promote if isinstance(member, Leader) else None
        if isinstance(member, Leader):
            member = member.member
        if member is None:
            return None
        ...
        return RecoveryTarget(
            TargetKind.REMOTE if isinstance(member, RemoteMember) else TargetKind.MEMBER,
            member.name,
            str(host) if host else None,
            str(conn_kwargs.get('port')) if conn_kwargs.get('port') else None,
            str(conn_kwargs.get('dbname')) if conn_kwargs.get('dbname') else None,
    ...
    def touch_member(self) -> bool:
        with self._member_state_lock:
            snapshot = self._node_state()
            data: Dict[str, Any] = {
                'conn_url': self.state_handler.connection_string,
                'api_url': self.patroni.api.connection_string,
                'state': self._pg_state(snapshot),
                'role': self._pg_role(snapshot),
            ...
            if self.is_leader() and not self.node.recovery().checkpoint_after_promote:
                data['checkpoint_after_promote'] = False
            tags = self.get_effective_tags()
            if tags:
                data['tags'] = tags
            pending_reason = self._pending_reason(snapshot)
            if pending_reason:
                data['pending_restart'] = True
                data['pending_restart_reason'] = pending_reason
            if self._async_executor.scheduled_action in (None, 'promote') \
                ...
                try:
                    ...
                    if not timeline:
                        ...
                        if self._pg_role(snapshot) == PostgresqlRole.STANDBY_LEADER:
                            timeline = pg_control_timeline or self.node.control_timeline()
                        else:
                            timeline = self.node.replica_timeline(self._leader_timeline) or 0
                        if timeline:
                            self._last_timeline = timeline
                    if timeline:
    ...
    def clone(self, clone_member: Union[Leader, Member, None] = None, msg: str = '(without leader)',
        ...
        mode = CloneMode.LEADER if clone_from_leader else CloneMode.CONFIGURED
        command = self._lifecycle(
            CommandKind.CLONE,
            DesiredRole.REPLICA,
            recovery_target=self._recovery_target(clone_member),
            clone_mode=mode,
        )
        if self._run_command(command):
            return self._follow_clone(msg)

        logger.error('failed to bootstrap %s', msg)

    def _follow_clone(self, msg: str) -> bool:
    ...
    def bootstrap(self) -> str:
        ...
        if self.cluster.is_unlocked() and self.cluster.initialize is None\
            ...
            if self.dcs.initialize(create_new=True):  # race for initialization
                ...
                if self.is_standby_cluster():
                    ...
                    ret = self._async_executor.try_run_async('bootstrap', self._run_command, args=(command,))
        ...
        can_bootstrap = self.node.can_clone(create_replica_methods)
    ...
    def _handle_crash_recovery(self) -> Optional[str]:
        if self._crash_recovery_started == 0 and (self.cluster.is_unlocked() or self.node.can_rewind()):
            self._crash_recovery_started = time.time()
            msg = 'doing crash recovery in a single user mode'
            command = self._lifecycle(CommandKind.CRASH_RECOVERY)
            return self._async_executor.try_run_async(msg, self._run_command, args=(command,)) or msg
    ...
    def _handle_rewind_or_reinitialize(self) -> Optional[str]:
        ...
        target = self._recovery_target(leader)
        if not self.node.rewind_needed(target) or not target:
            return None

        if self.node.can_rewind():
            # rewind is required, but postgres wasn't shut down cleanly.
            if not self._is_running() and \
                    self.node.controldata().get('Database cluster state') == 'in archive recovery':
                msg = self._handle_crash_recovery()
                if msg:
                    return msg

            msg = 'running pg_rewind from ' + target.name
            command = self._lifecycle(
                CommandKind.REWIND,
                DesiredRole.REPLICA,
        ...
        if self.node.recovery().remove_on_divergence and not self.is_standby_cluster():
    ...
    def recover(self) -> str:
        ...
        if timeout\
                ...
                and self._pg_state() == PostgresqlState.CRASHED\
                and self._pg_role() == PostgresqlRole.PRIMARY\
        ...
        if data.get('Database cluster state') in ('in production', 'shutting down', 'in crash recovery'):
            if self.node.restored_from_backup():
        ...
        if self.has_lock() and not self.is_standby_cluster():
            self._reset_recovery()  # we want to later trigger CHECKPOINT after promote
            msg = "starting as readonly because i had the session lock"
            node_to_follow = None
        else:
            if not self.node.recovery().executed:
    ...
    def follow(self, demote_reason: str, follow_reason: str, refresh: bool = True) -> str:
        ...
        if self.is_paused():
            ...
            if not (recovery.needed and (recovery.remove_on_divergence or self.node.can_rewind()))\
                ...
                elif self._pg_role() != PostgresqlRole.STANDBY_LEADER:
        ...
        if not self.is_paused():
            self._run_command(self._lifecycle(CommandKind.APPLY_CONFIG))
        ...
        if self.is_standby_cluster() and role == PostgresqlRole.REPLICA \
            ...
            config_change = self.node.check_recovery_conf(self._recovery_target(node_to_follow))
            change_required = config_change.change_required
            ...
            if change_required:
                ...
                if role == PostgresqlRole.STANDBY_LEADER and self._pg_role() != role:
                    ...
                    self._callback(CallbackKind.ROLE_CHANGE)
    ...
    def is_quorum_commit_mode(self) -> bool:
        ...
        return global_config.is_quorum_commit_mode and self._node_state().supports_multiple_sync
    ...
    def _process_multisync_prepromote(self) -> bool:
        ...
        if global_config.min_synchronous_nodes > 0:
            ...
            if self.cluster.sync.leader and not self._node_state().supports_multiple_sync and len(sync) > 1:
    ...
    def enforce_primary_role(self, message: str, promote_message: str) -> str:
        ...
        if self.node.is_primary():
            ...
            return message
        elif self._pg_role() in (PostgresqlRole.PRIMARY, PostgresqlRole.PROMOTED):
            self.process_sync_replication()
            return message
        else:
            ...
            if self._pg_role() not in (PostgresqlRole.PRIMARY, PostgresqlRole.PROMOTED):
    ...
    def update_failsafe(self, data: Dict[str, Any]) -> Union[int, str, None]:
        ...
        """
        if self._pg_state() == PostgresqlState.RUNNING and self._pg_role() == PostgresqlRole.PRIMARY:
    ...
    def _is_healthiest_node(self, members: Collection[Member],
        ...
        my_timeline = self.node.replica_timeline(cluster_timeline)
        if my_timeline:
            self._last_timeline = my_timeline
        my_wal_position = self.node.last_operation()
    ...
    def is_healthiest_node(self) -> bool:
        ...
        if self._is_starting():  # postgresql still starting up is unhealthy
            return False

        if self.node.is_primary():
            if self.is_paused():
                # in pause leader is the healthiest only when no initialize or sysid matches with initialize!
                return not self.cluster.initialize or self._node_state().system_identifier == self.cluster.initialize
    ...
    def demote(self, mode: str) -> Optional[bool]:
        ...
        if mode == 'demote-cluster' and self.node.archive_ready():
        ...
        if mode == 'demote-cluster':
            if demote_cluster_with_archive:
                self._run_command(self._lifecycle(CommandKind.ARCHIVE_WAL))
            else:
        ...
        if mode_control['async_req']:
            ...
            if self.node.rewind_needed(self._recovery_target(leader)):
    ...
    def process_manual_failover_from_leader(self) -> Optional[str]:
        ...
        # then do nothing
        if not failover or (self.is_paused() and not self.node.is_primary()):
            return
    ...
    def process_unhealthy_cluster(self) -> str:
        ...
        if not self.is_paused() and not self.is_standby_cluster() and \
            ...
            if time_left > 0 and self.node.replication_state() == 'streaming' and \
                    self.node.last_operation() > self._prev_wal_lsn or \
    ...
    def process_healthy_cluster(self) -> str:
        if self.has_lock():
            if self.is_paused() and not self.node.is_primary():
            ...
            if self.update_lock(True):
                ...
                if self.is_standby_cluster():
                    ...
                    msg = 'no action. I am ({0}), the standby leader with the lock'.format(self.state_handler.name) \
                          if self._pg_role() == PostgresqlRole.STANDBY_LEADER else \
    ...
    def evaluate_scheduled_restart(self) -> Optional[str]:
        ...
        if restart_data:
            recent_time = self.node.postmaster_start()
    ...
    def restart_matches(self, role: Optional[str], postgres_version: Optional[str], pending_restart: bool) -> bool:
        ...
        if role and role != self._pg_role():
            reason_to_cancel = "host role mismatch"

        if postgres_version and postgres_version_to_int(postgres_version) <= int(self.node.server_version()):
            reason_to_cancel = "postgres version mismatch"

        if pending_restart and not self._node_state().pending_restart:
    ...
    def schedule_future_restart(self, restart_data: Dict[str, Any]) -> bool:
        with self._async_executor:
            restart_data['postmaster_start_time'] = self.node.postmaster_start()
    ...
    def _do_reinitialize(self, cluster: Cluster, from_leader: bool = False) -> Optional[bool]:
        ...
        if clone_member:
            ...
            command = self._lifecycle(
                ...
                timeout=self.patroni.config['retry_timeout'],
                recovery_target=self._recovery_target(clone_member),
                clone_mode=CloneMode.LEADER if from_leader else CloneMode.CONFIGURED,
                divergence=DivergencePolicy.REINITIALIZE,
            )
            if self._run_command(command):
                return self._follow_clone(msg)
    ...
    def handle_long_action_in_progress(self) -> str:
        ...
        if self.has_lock() and self.update_lock():
            if self._async_executor.scheduled_action == 'doing crash recovery in a single user mode':
                ...
                if time_left <= 0 and self.is_failover_possible():
                    ...
                    self.node.cancel(CancelMode.KILL)
                    self.demote('immediate')
        ...
        elif not self.node.recovery().bootstrapping and not self.is_paused():
    ...
    def post_recover(self) -> Optional[str]:
        if not self._is_running():
            self.watchdog.disable()
            if self.has_lock():
                if self._pg_role() in (PostgresqlRole.PRIMARY, PostgresqlRole.STANDBY_LEADER):
                    self.state_handler.set_role(PostgresqlRole.DEMOTED)
                    self._callback(CallbackKind.ROLE_CHANGE)
                self._delete_leader()
    ...
    def cancel_initialization(self) -> None:
        ...
        self._run_command(self._lifecycle(CommandKind.MOVE_DATA))
        raise PatroniFatalException('Failed to bootstrap cluster')

    def _post_bootstrap(self) -> Optional[bool]:
        result = self._run_command(self._lifecycle(CommandKind.POST_BOOTSTRAP, DesiredRole.PRIMARY))
        with self._async_response:
            self._async_response.complete(result)
    ...
    def post_bootstrap(self) -> str:
        ...
        if not self._is_running() or result is False:
        ...
        if result is None:
            ...
            self.state_handler.set_role(PostgresqlRole.PRIMARY)
            ret = self._async_executor.try_run_async('post_bootstrap', self._post_bootstrap)
            return ret or 'running post_bootstrap'

        self._run_command(self._lifecycle(CommandKind.SET_BOOTSTRAP, bootstrap_state=BootstrapState.IDLE))
    ...
    def handle_starting_instance(self) -> Optional[str]:
        ...
        if not self.state_handler.check_for_startup() or self.is_paused():
            ...
            if self.is_paused():
                self.state_handler.set_state(PostgresqlState.RUNNING if self._is_running()
        ...
        try:
            try:
                ...
                if self.is_failsafe_mode():
                    ...
                    try:
                        self._last_wal_lsn = self.node.last_operation()
            ...
            if self.recovering:
                ...
                if not self.node.recovery().needed:
                ...
                self._crash_recovery_started = 0
                recovery = self.node.recovery()
                if recovery.executed and not recovery.failed:
                    self._reset_recovery()
            ...
            try:
                data_directory_is_empty = self.node.data_empty()
            ...
            if not data_directory_is_accessible or data_directory_is_empty:
                ...
                data_sysid = self._node_state().system_identifier
                ...
                if self.sysid_valid(self.cluster.initialize):
                    if self.cluster.initialize != data_sysid:
                        if self.is_paused():
                            ...
                            sys.exit(1)
                elif self.cluster.is_unlocked() and not self.is_paused() and not self.node.recovery().callback_called:
                    # "bootstrap", but data directory is not empty
                    if self._is_running() and not self.node.is_primary():
            ...
            if not self.state_handler.is_healthy():
                if self.is_paused():
                    ...
                    elif recovery.failed or not recovery.executed and not \
                            (recovery.needed and (recovery.remove_on_divergence or self.node.can_rewind())):
                        return 'postgres is not running'

                if self._pg_state() in (PostgresqlState.RUNNING, PostgresqlState.STARTING):
            ...
            if (not self._async_executor.busy or is_promoting) and not self._is_starting():
                create_slots = self._sync_replication_slots(False)

                if not self.node.recovery().callback_called:
                    if not is_promoting and not self.node.is_primary():
                        self._trigger_recovery()
                    self._callback(CallbackKind.START)
    ...
    def _handle_dcs_error(self) -> str:
        if not self.is_paused() and self._is_running():
            if self.node.is_primary():
